...
start(){
    ...
    then
         ...
         java ${JAVA_OPS} -classpath /etc/nebula/:/etc/nebula/babels/:/etc/nebula/offline/:$(echo ${APP_HOME}/lib/* | tr ' ' ':') ${MAIN_CLASS} $ARGS 1>${APP_HOME}/logs/log.txt 2>${APP_HOME}/logs/err.txt &
        pid=$(_pid)
        if [ -n "$pid" ]
        then
